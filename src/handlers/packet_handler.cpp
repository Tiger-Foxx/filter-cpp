...
bool PacketHandler::Initialize() {
    ...
    if (!nfq_handle_) {
        ...
        std::cerr << "   Make sure you have CAP_NET_ADMIN capability" << std::endl;
        return false;
    }
    ...
    // Create queue
    queue_handle_ = nfq_create_queue(nfq_handle_, queue_num_, &PacketCallback, this);
    if (!queue_handle_) {
    ...
    if (nfq_set_mode(queue_handle_, NFQNL_COPY_PACKET, 0xffff) < 0) {
        std::cerr << "❌ Error: Cannot set copy mode" << std::endl;
        return false;
    }
...
void PacketHandler::Start(std::function<void(bool)> callback) {
    ...
    while (running_.load()) {
        try {
            ...
            if (received_bytes < 0) {
                if (errno == ENOBUFS) {
                    std::cerr << "⚠️  Warning: Packet buffer overrun - increase buffer size" << std::endl;
                    continue;
                }
            ...
            if (received_bytes == 0) {
                std::cerr << "⚠️  Warning: Received 0 bytes" << std::endl;
                continue;
            }

            // Handle the netlink message
            nfq_handle_packet(nfq_handle_, buffer, received_bytes);

        } catch (const std::exception& e) {
            std::cerr << "❌ Exception in packet processing: " << e.what() << std::endl;
        } catch (...) {
            std::cerr << "❌ Unknown exception in packet processing" << std::endl;
        }
    }
...
int PacketHandler::HandlePacket(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
    ...
    uint64_t packet_id = total_packets_.load();
    ...
    try {
        ...
        if (packet_id % 1000 == 0) {
            ...
            auto reassembled = reassembled_packets_.load();
...
bool PacketHandler::NeedsHTTPReassembly(const PacketData& packet) {
    // Check if TCP traffic on HTTP ports
...
void PacketHandler::HandleTCPReassembly(unsigned char* data, int len, PacketData& packet) {
    ...
    // Try to reassemble and get HTTP data
...
uint64_t PacketHandler::GetConnectionKey(const PacketData& packet) {
    ...
    // Create a simple hash-based connection key
...
void PacketHandler::BlockConnection(uint64_t connection_key) {
    ...
    if (blocked_connections_.size() > 50000) {
        // Remove some old connections (simple approach)
...
PacketHandler::Stats PacketHandler::GetStats() const {
    ...
    stats.dropped_packets = dropped_packets_.load();
    stats.accepted_packets = accepted_packets_.load();
    stats.reassembled_packets = reassembled_packets_.load();
